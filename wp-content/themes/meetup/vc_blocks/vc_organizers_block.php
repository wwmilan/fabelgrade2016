 ...
 * The Shortcode
 */
function ebor_organizers_shortcode( $atts ) {
	extract( 
		shortcode_atts( 
			array(
				'type' => 'grid',
	...
	if (!( $filter == 'all' )) {
		if( function_exists( 'icl_object_id' ) ){
			$filter = (int)icl_object_id( $filter, 'organizers_category', true);
		}
...
add_shortcode( 'organizers', 'ebor_organizers_shortcode' );

/**
 * The VC Functions
 */
function ebor_organizers_shortcode_vc() {
	
	vc_map( 
		array(
			"icon" => 'meetup-vc-block',
			"name" => __("Organizers Feed", 'meetup'),
			"base" => "organizers",
			"category" => __('Meetup - WP Theme', 'meetup'),
			'description' => 'Add your organizers posts to the page.',
			"params" => array(
				array(
					"type" => "textfield",
...
add_action( 'vc_before_init', 'ebor_organizers_shortcode_vc');
